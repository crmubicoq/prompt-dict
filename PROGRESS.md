...
>
> **다음은 P1.** 최대 병목은 코드가 아니라 등록 마찰이므로
> T-104(일괄 붙여넣기)와 T-101(카테고리 필수 해제)이 최우선이다.
> P1이 끝나면 D1(실데이터 수집 1회차)로 넘어간다.

### P1 — 버그 및 보안 수정

- [ ] `escapeHtml()` 유틸 추가, 카드 렌더링 전면 적용 (B1)
- [ ] 인라인 `onclick` → 이벤트 위임(delegation)으로 교체 (B2)
- [ ] `exportData()` 전체 데이터 포함하도록 확장 + `version: '2.0'` (B3)
- [ ] `importData()` 가 v1.0 / v2.0 둘 다 처리 (하위 호환)
- [ ] ID 생성을 `crypto.randomUUID()` 로 교체, 기존 숫자 ID 마이그레이션 (B5)
- [ ] `updatedAt` 추가, 정렬 옵션 "수정순" 추가 (B6)
- [ ] `favoriteIds` 단일 소스로 통일, `isFavorite` 필드 제거 (B7)
- [ ] 검색 범위에 `notes` 포함 (B8)
- [ ] 임포트 파서 3종에 `notes` 필드 추가 (B14)
...
- [ ] 내보내기 → 브라우저 데이터 전체 삭제 → 불러오기 시
      프롬프트·카테고리·즐겨찾기·참고자료·**메모**가 전부 복원됨
- [ ] 제목·태그에 `<`, `'`, `"` 입력해도 화면·필터가 정상
- [ ] 프롬프트 100개 상태에서 검색·정렬·필터 지연 없음
- [ ] 썸네일 3장 이상 저장해도 쿼터 실패 없음
- [ ] **`localStorage` 직접 호출 0건** (전부 `Storage` 경유) ← 서버 이식 가능 여부를 결정
