...
- [x] `updatedAt` 추가, 정렬 옵션 "수정순" 추가 (B6) — T-117
- [x] `favoriteIds` 단일 소스로 통일, `isFavorite` 필드 제거 (B7) — T-118
- [x] 검색 범위에 `notes` 포함 (B8) — T-119
- [x] 샘플 데이터 자동 주입 중단 (B12) — T-120
- [x] TXT 구분자를 충돌 없는 마커로 (B16) — T-121
- [x] `Esc` 키 전역 모달 닫기 (B17) — T-123

**남은 것 — 둘 다 보류**

- [ ] CSV 파서를 따옴표·개행 처리 가능하게 재작성 (B13) — 소스에 엑셀 없어 후순위
- [ ] 검색 디바운스 200ms (B18) — 36건에서 버벅이지 않는다. 150건에서 판단
- [ ] 썸네일 노출 조건을 체크박스로 (B15) — 이미지 소스가 P4 이후다

**1-C** — T-130 Codex 검수를 남기고 P1 이 닫힌다.
